...
> **Note:** This design document was written before implementation. During development,
> `ureq` was replaced with `minreq` for smaller binary size, and the `dirs` dependency
> was removed in favor of a built-in `cache_dir()` implementation.
